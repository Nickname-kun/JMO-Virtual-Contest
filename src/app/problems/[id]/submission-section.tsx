      ...
      try {
        ...
        const normalizeLatexFraction = (latex: string): string => {
          ...
          const normalizedLatex = cleanedLatex
            ...
            .replace(/_([^C]+)C_([^}]+)/g, 'combinations($1,$2)') // _nC_k -> combinations(n,k)
            .replace(/_([^C]+)C([0-9]+)/g, 'combinations($1,$2)') // _nCk -> combinations(n,k)
            .replace(/\\\\frac\{([^}]+)\}\{([^}]+)\}/g, '($1)/($2)')
          ...
          <VStack spacing={4} align="stretch">
            <FormControl>
              ...
              <HStack spacing={2} mb={2} wrap="wrap">
                ...
                <Button size="sm" onClick={() => insertLatex('_#0C_#?}')}>コンビネーション(_nC_k) <InlineMath math="□C□" /></Button>
